...
type Settings struct {
	...
	RosterPositions  []RosterPosition `xml:"roster_positions"`
...
type StatModifiers struct {
	Stats struct {
		Text          string         `xml:",chardata"`
		StatModifiers []StatModifier `xml:"stat"`
	} `xml:"stats"`
}
...
// Games because i got lazy
type GameFantasyContent struct {
	XMLName     xml.Name `xml:"fantasy_content"`
	Text        string   `xml:",chardata"`
...
// year.
func (c *Client) GetUserLeagues(gameKey string) ([]League, error) {
...
// GetLeagueSettings returns all league settings.
func (c *Client) GetLeagueSettings(leagueKey string) (*Settings, error) {
	content, err := c.GetFantasyContent(
		fmt.Sprintf("%s/league/%s/settings", YahooBaseURL, leagueKey))
	if err != nil {
		return nil, err
	}
...
// GetLeagueSettings returns all league settings.
func (c *Client) GetUserGames() ([]Game, error) {
	content, err := c.GetFantasyContent(
		fmt.Sprintf("%s/users;use_login=1/games", YahooBaseURL))
	if err != nil {
		return nil, err
	}
	...
	return content.Users[0].Games, nil
	return content.Games, nil
}
